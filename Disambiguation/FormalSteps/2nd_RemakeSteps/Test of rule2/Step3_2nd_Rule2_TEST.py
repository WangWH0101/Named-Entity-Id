...
def PairJudge1(i_org,j_org): #input the name of the organization pair i&j
    ...
    if len(a)!=len(b) and shared >= 2: #not equal and shared >2 (rule 2 modified) !!!!!!!!!Problem!!!!!!!!
        Idf=1
        return Idf
...
def rule2(i_org,j_org):
    ...
    if len(a)!=len(b) and shared>=2: #not equal and shared >2 (rule 2 modified) !!!!!!!!!Problem!!!!!!!!
        Idf=1
    return Idf
    #It's apparent that this rule can easily misclassify intitutions
    '''
    For example:
...
fp0 = open('/mnt/sdb/wos2018-wwh/Disambiguation/ScreenRst/TEST/Rule2_2_NewMod2_'+TGF+'_OrgPair_Threshold_'+str(THRESHOLD)+'_'+str(STY)+'to'+str(EDY)+'.pickle', 'wb')
...
file1=open('/mnt/sdb/wos2018-wwh/Disambiguation/Result_2nd/TEST_rule2/TEST_rule2_2_NewMod2_Threshold_'+str(THRESHOLD)+'_'+TGF+'_'+str(STY)+'to'+str(EDY)+'.pickle', 'wb')
