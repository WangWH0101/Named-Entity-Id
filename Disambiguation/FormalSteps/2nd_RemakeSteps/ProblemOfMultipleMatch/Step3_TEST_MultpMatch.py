...
def MulMatch(ISSGrp):
    InsNMGrp = set()
    for x in ISSGrp.keys():
        InsNMGrp = InsNMGrp | ISSGrp[x]
    for InsNM in InsNMGrp:
        RepNM = get_keys1(ISSGrp, InsNM)
        if len(RepNM) > 1:
            return RepNM
...
if type(MulMatch(InD)) is list:
    print('!!!!!!!!!!!Multiple Match Clean Failed!!!!!!!!!!')
