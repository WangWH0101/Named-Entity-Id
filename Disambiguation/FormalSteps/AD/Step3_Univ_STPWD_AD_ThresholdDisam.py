...
3.Stop words are used in this version for rule 4 only
4.This version will only consider universities instead of all institutions
'''
...
for nm0 in AuAd.keys():
	...
	for i in range(addrlen):
		...
		for j in range(i+1,addrlen):
			if org[j]=='*' or org[j]=='' or AuAd[nm0]['Country'][j]=='*' or AuAd[nm0]['Country'][j]=='':
				continue
			elif UnivWD[0] not in org[j] and UnivWD[1] not in org[j]:
			...
			elif len(a)!=len(b) and shared>2: #not equal and shared >2 (rule 2 modified)
				AuAdPOOL[nm0]['C'].append('_'.join([str(i),str(j)]))
			...
			elif Levenshtein.jaro_winkler(i_Nspc,j_Nspc)>0.8: #the edit distance<0.2 (rule 4:'http://www.alias-i.com/lingpipe/docs/api/com/aliasi/spell/JaroWinklerDistance.html')
				AuAdPOOL[nm0]['C'].append('_'.join([str(i),str(j)]))
	...
	for cp in AuAdPOOL[nm0]['C']: #notice that all the unknown information is now represented by '*'/''
		...
		'''
		In order to avoid the universities with same name from different countries are clssified into the same group, 
		the country information of each university pair are added into the D set right after their sequence number
		'''
		...
		if i_segnum!=j_segnum or i_segnum<=3: #rule 6
			if AuAd[nm0]['SubOrganization'][i]==AuAd[nm0]['SubOrganization'][j] and AuAd[nm0]['SubOrganization'][i]!='*' and AuAd[nm0]['SubOrganization'][i]!='':
				AuAdPOOL[nm0]['D'].append('_'.join([str(i),str(j),AuAd[nm0]['Country'][i]]))
		elif i_segnum>3:
...
fp0 =open('/mnt/sdb/wos2018-wwh/Disambiguation/ScreenRst/AD_Univ_CandDset_'+str(STY)+'to'+str(EDY)+'.pickle', 'wb')
...
for nm1 in AuAdPOOL:
	...
	for cp in AuAdPOOL[nm1]['D']: #the institution couple in group 'D' within an author block 'nm1'
		i=int(cp.split('_')[0]) #the sequence number of 'i' mentioned above
		j=int(cp.split('_')[1]) #the sequence number of 'j' mentioned above
		Ctry=cp.split('_')[2] #the country of the institution pair
		fd=0 #Identifier:whether this pair of organization already exists
		#print('Org_couple ',cp,' :',i,',',j,'\n')
		if subseq==0:
			subPair[subseq]=[org[i],org[j],Ctry]
			subseq+=1
		else:
			for x in subPair.keys():
				if org[i]==subPair[x][0] and org[j]==subPair[x][1] and Ctry==subPair[x][2]: #continue if this pair already exits
					fd=1
					continue
				elif org[i]==subPair[x][1] and org[j]==subPair[x][0] and Ctry==subPair[x][2]: #continue if this pair already exits
					fd=1
					continue
			if fd==0:
				subPair[subseq]=[org[i],org[j],Ctry]
	...
	for x in subPair.keys():
		...
		if seq==0:
			ThrMtrix[seq]=[org_i,org_j,Ctry,1]
			seq+=1
		else:
			for y in ThrMtrix.keys():
				if org_i==ThrMtrix[y][0] and org_j==ThrMtrix[y][1] and Ctry==ThrMtrix[y][2]:
					#print('i=i,j=j:')
					#print(ThrMtrix[y],'\n')
					ThrMtrix[y][3]+=1
					fd=1
				elif org_i==ThrMtrix[y][1] and org_j==ThrMtrix[y][0] and Ctry==ThrMtrix[y][2]:
					#print('i=j,j=i:')
					#print(ThrMtrix[y],'\n')
					ThrMtrix[y][3]+=1
					fd=1
...
fp0=open('/mnt/sdb/wos2018-wwh/Disambiguation/ScreenRst/AD_Univ_OrgPair_2008to2016.pickle','wb')
...
file1=open('/mnt/sdb/wos2018-wwh/Disambiguation/Result/AD_Univ_IND_Threshold_'+str(THRESHOLD)+'_'+str(STY)+'to'+str(EDY)+'.pickle', 'wb')
